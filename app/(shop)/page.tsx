...
export default function ShopHomePage() {
  ...
  const filteredProducts = useMemo(() => {
    return allVariants.filter(v => {
      ...
      return matchesCat && matchesBrand && (!searchTerm || v.part_number?.toLowerCase().includes(searchLower) || v.sku?.toLowerCase().includes(searchLower) || v.products?.name?.toLowerCase().includes(searchLower));
    });
    ...
    <div className="flex gap-8 max-w-7xl mx-auto p-4 md:p-8 animate-in fade-in duration-700">
      ...
      <aside className="w-64 hidden md:block shrink-0 space-y-6">
        <div className="bg-white rounded-2xl border border-slate-200 p-5 sticky top-28 shadow-sm">
          ...
          <ul className="space-y-1 mb-8">
            <li onClick={() => setSelectedCat(null)} className={clsx("px-3 py-2 rounded-xl text-xs font-bold uppercase cursor-pointer transition-all active:scale-95", !selectedCat ? "bg-blue-600 text-white shadow-lg" : "text-slate-500 hover:bg-slate-50")}>All Items</li>
          ...
          <div className="my-6 border-t border-slate-100 pt-6"><div className="flex items-center gap-2 mb-4 text-slate-800"><Tags size={16} className="text-orange-500" /><h3 className="text-[11px] font-bold uppercase tracking-widest">Brands</h3></div>
      ...
      <div className="flex-1 space-y-6">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
           ...
           <input type="text" placeholder="Instant Search Part Number or Description..." className="w-full bg-transparent outline-none text-sm font-medium" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
           {searchTerm && <X size={18} className="text-slate-300 cursor-pointer" onClick={() => setSearchTerm("")} />}
        </div>

        {loading ? (
          <div className="flex flex-col items-center justify-center py-40 gap-4"><Spinner size={40} className="text-blue-600" /><p className="text-[10px] font-bold uppercase tracking-[0.3em] animate-pulse">Syncing Inventory...</p></div>
        ) : (
                ...
                <div key={v.id} className="bg-white rounded-3xl border border-slate-200 hover:border-blue-500 transition-all duration-300 group flex flex-col h-full relative overflow-hidden shadow-sm hover:shadow-xl">
                  ...
                  >
                    {v.products?.image_url ? (
                      <img src={v.products.image_url} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                    ) : (
                      <Package size={60} strokeWidth={1} className="text-slate-200 group-hover:scale-110 transition-transform duration-700" />
                    )}
                    
                    {isStockOut && <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex items-center justify-center z-10 font-bold uppercase text-[10px] tracking-widest">Out of Stock</div>}
                    {hasDiscount && (
                      <div className="absolute top-4 right-4 bg-red-600 text-white text-[10px] font-black px-2 py-1 rounded-lg z-20 shadow-lg">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    {/* Name Clicks to Detail */}
                    <h3 
                      onClick={() => setSelectedProduct(v)}
                      className="text-sm font-bold text-slate-800 line-clamp-2 h-10 leading-tight mb-1 uppercase italic tracking-tighter cursor-pointer hover:text-blue-600 transition-colors"
                    >
                      {v.products?.name}
                    </h3>
                    <p className="text-[10px] font-bold text-slate-400 mb-4 uppercase tracking-widest">PN: {v.part_number}</p>
                    ...
                    <div className="mt-auto flex items-end justify-between pt-4 border-t border-slate-50">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); addToCart(v.id); }} disabled={isStockOut} className="bg-[#2563EB] text-white p-4 rounded-2xl shadow-xl shadow-blue-100 active:scale-90 hover:bg-blue-700 transition-all disabled:opacity-30"><ShoppingCart size={22} strokeWidth={2.5} /></button>
                    </div>
                  </div>
                </div>
              );
